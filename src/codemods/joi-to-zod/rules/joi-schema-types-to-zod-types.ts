...
const SCHEMA_TYPE_TO_ZOD_TYPE: Record<string, string> = {
  Schema: ZOD_TYPE,
  SchemaLike: ZOD_TYPE,
  AnySchema: ZOD_TYPE,
...
async function joiSchemaTypesToZodTypes(modifications: Modifications): Promise<Modifications> {
  ...
  const edits = compactMap(root.findAll({ rule: { kind: 'nested_type_identifier' } }), node => {
    ...
    if (qualifier !== joiImportIdentifierName || name == null || rest.length > 0) return undefined;

    const zodType = SCHEMA_TYPE_TO_ZOD_TYPE[name];
    if (zodType == null) return undefined;

    const parent = node.parent();
    if (parent?.kind() !== 'generic_type') return node.replace(zodType);
